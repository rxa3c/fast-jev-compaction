...
import { appendCodexTrace, } from './codex-trace.js';
import { buildCodexPlan, parseCodexRollout, renderCodexRecoveryNote, } from './codex.js';
...
function finiteEnvironmentNumber(environment, key, fallback) {
...
export function resolveCodexHookConfig(environment = process.env) {
    const goal = environment.FAST_JEV_GOAL?.trim();
    return {
        model: environment.FAST_JEV_MODEL?.trim() || DEFAULT_MODEL,
        goal: goal || undefined,
        minReductionRatio: boundedRatio(finiteEnvironmentNumber(environment, 'FAST_JEV_MIN_REDUCTION_RATIO', 0.25)),
        noteResultChars: nonNegativeInteger(finiteEnvironmentNumber(environment, 'FAST_JEV_NOTE_RESULT_CHARS', 800)),
        keepThreshold: boundedRatio(finiteEnvironmentNumber(environment, 'FAST_JEV_KEEP_THRESHOLD', 0.5)),
        preserveRecentMessages: nonNegativeInteger(finiteEnvironmentNumber(environment, 'FAST_JEV_PRESERVE_RECENT_MESSAGES', 6)),
        maxStateTokens: Math.max(1, finiteEnvironmentNumber(environment, 'FAST_JEV_MAX_STATE_TOKENS', 25_000)),
        maxRequestTokens: Math.max(1, finiteEnvironmentNumber(environment, 'FAST_JEV_MAX_REQUEST_TOKENS', 30_000)),
        truncateHeadChars: nonNegativeInteger(finiteEnvironmentNumber(environment, 'FAST_JEV_TRUNCATE_HEAD_CHARS', 300)),
    };
}
export function parseCodexHookInput(text) {
...
function pendingPath(input, environment, dependencies) {
    const root = dependencies.dataRoot ||
        environment.PLUGIN_DATA ||
        join(tmpdir(), 'fast-jev-codex');
    return join(root, 'sessions', safeSessionId(input.session_id), 'pending.json');
}
async function writePending(path, state) {
...
async function savePendingBestEffort(path, state) {
    try {
...
function planOptions(config, input, createdAt) {
    return {
        goal: config.goal,
...
async function emitTrace(input, environment, dependencies, event, fields = {}) {
    if (!dependencies.trace)
        return;
    try {
...
function tracedAsker(asker, input, environment, dependencies, model) {
    if (!dependencies.trace)
        return asker;
    return {
...
async function runPreCompact(input, environment, dependencies) {
    ...
    try {
        const readRollout = dependencies.readRollout ?? ((path) => readFile(path, 'utf8'));
        ...
        const plan = await buildCodexPlan(lines, tracedAsker(asker, input, environment, dependencies, config.model), planOptions(config, input, createdAt));
...
function boundedNote(note) {
    if (note.length <= RECOVERY_NOTE_LIMIT)
        return note;
...
async function runCompactSessionStart(input, environment, dependencies) {
    ...
    if (pending.status !== 'ready' || !pending.note) {
        ...
        return {
            systemMessage: 'fast-jev-codex: native Codex compaction completed without a Jev recovery note.',
        };
    }
...
async function runSessionStart(input, environment, dependencies) {
...
export async function runCodexHook(input, environment = process.env, dependencies = {}) {
    const effectiveEnvironment = environment === process.env
        ? await loadConfigEnvironment(environment)
        : environment;
    const tracedDependencies = dependencies.trace || environment !== process.env
        ? dependencies
