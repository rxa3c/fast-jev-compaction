...
function configPaths(environment) {
    ...
    return [...new Set(candidates.filter((path) => path !== undefined))];
}
/** Loads local config files while keeping explicitly supplied environment values authoritative. */
export async function loadConfigEnvironment(environment = process.env) {
    const fromFiles = {};
